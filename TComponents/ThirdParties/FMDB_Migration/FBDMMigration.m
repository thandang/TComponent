...
+ (instancetype) migration {
    static FMDBMigration *migrationInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!migrationInstance) {
            migrationInstance = [[FMDBMigration alloc] init];
        }
    });
...
- (void)createTable:(NSString *)tableName primaryKey:(NSString *)primaryKey { //Custom
    NSString *sql = [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@ (%@ varchar primary key)", tableName, primaryKey];
    [db_ executeUpdate:sql];
}

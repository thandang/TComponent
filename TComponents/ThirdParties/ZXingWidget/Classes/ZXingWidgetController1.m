  ...
  if (self) {
    ...
    decoding = NO;
      ...
      if ([[[UIDevice currentDevice] systemVersion] compare:@"7.0" options:NSNumericSearch] != NSOrderedAscending) {
          overlayRect.origin.y += 20;
//          overlayRect.size.height += 20;
      }
    OverlayView *theOverLayView = [[OverlayView alloc] initWithFrame:overlayRect
...
- (void)viewDidAppear:(BOOL)animated {
  [super viewDidAppear:animated];
    self.isStatusBarHidden = YES;//[[UIApplication sharedApplication] isStatusBarHidden];
...
//  if (false) {
  if (true) { //modify by Than Dang on May 27, 2014

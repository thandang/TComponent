...
- (NSString *) stringTrimedTailAndHead {
    if (!self) {
        return @"";
    }
    if ([self rangeOfString:@"\""].location != NSNotFound) {
        NSString *leadingTrailingWhiteSpacesPattern = @"(?:^\\s+)|(?:\\s+$)";
        NSRegularExpression *expression = [NSRegularExpression regularExpressionWithPattern:leadingTrailingWhiteSpacesPattern options:NSRegularExpressionCaseInsensitive error:NULL];
        NSRange stringRange = NSMakeRange(0, self.length);
        NSString  *trimmedString = [expression stringByReplacingMatchesInString:self options:NSMatchingReportProgress range:stringRange withTemplate:@"$1"];
